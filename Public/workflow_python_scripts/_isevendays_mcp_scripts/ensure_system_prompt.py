...
def load_default_prompt(default_prompt_path):
    ...
    Args:
        default_prompt_path (str): Path to the default prompt file.

    Returns:
        str: Default system prompt string.
    """
    try:
...
def Invoke(messages, **kwargs):
    ...
    # Default to DEFAULT_MCPO_URL (user-config MCPO_URL > env > localhost:8889) so tool
    # discovery uses the same MCPO base URL as execution; a hardcoded literal here would
    # ignore a user-configured MCPO_URL on the discovery side only.
    mcpo_url = kwargs.get("mcpo_url", DEFAULT_MCPO_URL)
    
    # user_identified_services can arrive as a streamed generator from an upstream node;
    # aggregate it to a string before use, falling back to empty if it is neither string
    # nor aggregatable.
