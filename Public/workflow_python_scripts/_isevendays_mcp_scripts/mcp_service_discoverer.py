...
class MCPServiceDiscoverer:
    ...
    def __init__(self, mcpo_url: str = DEFAULT_MCPO_URL):
        ...
        Args:
            mcpo_url (str): Base URL for the MCPO server. Defaults to DEFAULT_MCPO_URL.
        """
        self.mcpo_url = mcpo_url
        logger.info(f"MCPServiceDiscoverer initialized with MCPO URL: {self.mcpo_url}")

    def is_native_service(self, service_name: str) -> bool:
        """Check whether a service name maps to a native MCP server registry entry.

        A service is "native" when a `Public/Configs/MCPServers/<name>.json` config
        exists; such services are discovered through the official `mcp` SDK instead
        of the legacy MCPO OpenAPI proxy.

        Args:
            service_name (str): Name of the service to check.

        Returns:
            bool: True if a native MCP server config exists for the name.
        """
        try:
            return os.path.isfile(get_config_path('MCPServers', service_name))
        except Exception as e:
            logger.warning(f"Could not check MCPServers registry for '{service_name}': {e}")
            return False

    def discover_native_tools(self, service_name: str) -> Dict[str, Dict]:
        """Discover tools for a service through the native `mcp` SDK.

        Uses the shared `MCPClient.list_tools` (the same client backing the
        deterministic `MCPToolCall` node) and tags each entry with
        `"transport": "mcp"` so the executor routes calls through the SDK instead
        of the legacy MCPO HTTP path.

        Args:
            service_name (str): Name of an MCP server config under `Public/Configs/MCPServers/`.

        Returns:
            Dict[str, Dict]: Mapping tool name to details (service, transport, llm_schema),
            or an empty dict if discovery fails.
        """
        from Middleware.workflows.tools.mcp_client_tool import MCPClient, MCPToolCallError

        try:
            tools = MCPClient().list_tools(service_name)
        except MCPToolCallError as e:
            logger.error(f"Native MCP tool discovery failed for service '{service_name}': {e}")
            return {}
        except Exception as e:
            logger.exception(f"Unexpected error during native MCP discovery for '{service_name}': {e}")
            return {}

        native_tools_map = {}
        for tool_name, details in tools.items():
            native_tools_map[tool_name] = {
                "service": service_name,
                "transport": "mcp",
                "llm_schema": details.get("llm_schema", {}),
            }
        logger.info(f"Discovered {len(native_tools_map)} tools for native MCP service '{service_name}'.")
        return native_tools_map

    def fetch_service_schema(self, service_name: str) -> Optional[Dict]:
        """Fetch the OpenAPI schema for a single service.

        Args:
            service_name (str): Name of the service (e.g., "time").

        Returns:
            Optional[Dict]: OpenAPI schema as dictionary, or None if fetching or parsing fails.
        """
    ...
    def validate_tool_schema(self, schema: Dict, service_name: str) -> bool:
        ...
        Args:
            schema (Dict): OpenAPI schema dictionary to validate.
    ...
    def create_llm_schema(self, endpoint_details: Dict, full_schema: Dict) -> Dict:
        ...
        Args:
            endpoint_details (Dict): Dictionary of a specific endpoint (e.g., schema['paths']['/path']['get']).
            full_schema (Dict): The complete OpenAPI schema (for resolving $ref).

        Returns:
            Dict: Containing the LLM-compatible function schema.
        """
    ...
    def validate_tool_definition(self, tool_schema: Dict) -> bool:
        ...
        Args:
            tool_schema (Dict): The LLM tool schema dictionary to validate.
    ...
    def process_service_schema(self, schema: Dict, service_name: str) -> Dict[str, Dict]:
        ...
        Args:
            schema (Dict): The validated OpenAPI schema dictionary.
            service_name (str): Name of the service this schema belongs to.

        Returns:
            Dict[str, Dict]: Mapping operationId to tool details (service, path, method, llm_schema).
    ...
    def discover_mcp_tools(self, service_names: List[str]) -> Dict[str, Dict]:
        ...
        Args:
            service_names (List[str]): List of MCP service names (e.g., ["time", "weather"]).

        Returns:
            Dict[str, Dict]: Mapping operationId to its execution details and LLM schema.
        ...
        for service_name in service_names:
            ...
            # Prefer the native MCP SDK when the service has an MCPServers/ registry
            # entry; otherwise fall back to the legacy MCPO OpenAPI proxy.
            if self.is_native_service(service_name):
                logger.info(f"Service '{service_name}' found in MCPServers registry; using native MCP discovery.")
                native_tools = self.discover_native_tools(service_name)
                if native_tools:
                    duplicates = [op_id for op_id in native_tools if op_id in all_tools_map]
                    if duplicates:
                        logger.warning(f"Duplicate tool name(s) found while processing '{service_name}': {duplicates}. Tools from '{service_name}' will overwrite existing ones.")
                    all_tools_map.update(native_tools)
                    logger.info(f"Added/updated {len(native_tools)} tools from native MCP service '{service_name}'.")
                else:
                    logger.warning(f"No tools discovered for native MCP service '{service_name}'.")
                continue
    ...
    def discover_and_validate_mcp_tools(self, service_names: List[str], validate_llm_schema: bool = False) -> Dict[str, Dict]:
        ...
        Args:
            service_names (List[str]): List of service names to discover tools for.
            validate_llm_schema (bool): If True, validate the generated LLM schema for each tool
                                 using `validate_tool_definition` and exclude invalid tools.

        Returns:
            Dict[str, Dict]: Mapping operationId to tool details. Only includes tools that
