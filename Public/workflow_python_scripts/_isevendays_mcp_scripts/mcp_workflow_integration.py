...
# Import the base exception from the dynamic module loader so errors raised
# here are the same class that run_dynamic_module catches and reports.
...
from Public.workflow_python_scripts._isevendays_mcp_scripts import mcp_tool_executor
...
def parse_string_messages(messages_str: str) -> List[Dict[str, str]]:
    ...
    Args:
        messages_str (str): String message to parse.

    Returns:
        List[Dict[str, str]]: Containing one message dictionary.
    """
    parsed_messages = []
...
def validate_node_type(node_type: str) -> bool:
    ...
    Args:
        node_type (str): The type to validate.
...
def _format_single_tool_result(result: Dict) -> str:
    ...
    Args:
        result (Dict): Dictionary containing 'tool_call' and 'result' keys.

    Returns:
        str: Formatted string for the single tool result.
    """
    formatted_string = ""
    ...
    if "error" in tool_result:
        ...
        # Restore any curly braces Wilmer escaped to sentinel tokens
        # (text_utils.escape_brackets_in_string) before adding to the response.
        cleaned_result_str = json.dumps(tool_result, indent=2)
        cleaned_result_str = return_brackets_in_string(cleaned_result_str)
...
def format_results_only(tool_results: List[Dict]) -> str:
    ...
    Args:
        tool_results (List[Dict]): List of tool results to format.

    Returns:
        str: Formatted string containing only the tool results, removing any markers.
    """
    if not tool_results:
        return ""
...
def _parse_tool_execution_map_static(raw_map: Union[str, Dict, Any]) -> Dict:
    ...
    Args:
        raw_map (Union[str, Dict, Any]): The tool execution map as a dict or JSON string.

    Returns:
        Dict: The parsed tool execution map.

    Raises:
        MCPConfigurationError: If the input is not a dict/string or cannot be parsed to a dict.
    """
...
def _parse_messages_input_static(raw_messages: Any) -> List[Dict[str, str]]:
    ...
    Args:
        raw_messages (Any): The raw messages input (list, JSON string, or plain string).

    Returns:
        List[Dict[str, str]]: The parsed list of message dictionaries.

    Raises:
        MCPMessageParsingError: If the input cannot be parsed into valid message dictionaries.
    """
...
class MCPWorkflowHandler:
    ...
    def __init__(self, *args, **kwargs):
        """Initialize the handler from pre-parsed arguments and validate them.

        Args:
            *args: Variable length positional argument list (currently unused for core data).
            **kwargs: Keyword arguments supplying pre-parsed messages, original_response,
                mcpo_url, tool_execution_map, validate_execution, and node_type.

        Raises:
            MCPConfigurationError: If configuration (e.g., node_type) is invalid.
            MCPMessageParsingError: If the messages input is invalid.
            MCPIntegrationError: For unexpected errors during initialization.
        """
    ...
    def _parse_arguments(self, *args, **kwargs):
        ...
        Args:
            *args: Variable length positional argument list (currently unused for core data).
            **kwargs: Keyword arguments supplying messages, original_response, mcpo_url,
                tool_execution_map, validate_execution, and node_type.
        """
    ...
    def _validate_inputs(self):
        ...
        Raises:
            MCPConfigurationError: If the node_type is provided but invalid.
        """
    ...
    def execute_tools(self) -> str:
        ...
        Returns:
            str: Containing formatted tool execution results, or the original
...
def Invoke(*args, **kwargs) -> str:
    ...
    Args:
        *args: Variable length argument list. arg[0] (raw_messages) is the messages
               list or string; arg[1] (original_response) is an optional LLM response.
        **kwargs: Arbitrary keyword arguments.
        messages (Any): Messages list or string.
        original_response (str): LLM response.
        mcpo_url (str): Base URL for the MCPO server.
        tool_execution_map (Union[str, Dict]): Map from operationId to execution details (can be dict or JSON string).
        validate_execution (bool): Whether to validate tool execution.
        node_type (str): Type of the node (for validation).

    Returns:
        str: Containing tool execution results or the original response.
    ...
    try:
        ...
        handler_kwargs['messages'] = parsed_messages
        handler_kwargs['original_response'] = original_response
        handler_kwargs['tool_execution_map'] = parsed_tool_execution_map
