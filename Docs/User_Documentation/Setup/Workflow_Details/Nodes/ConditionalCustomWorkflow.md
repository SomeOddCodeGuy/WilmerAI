...
| **`is_responder`**                       | Boolean          | No       | `false` | Determines if the output (from a sub-workflow or default content) provides the final user-facing response. (The original key name `returnToUser` is still honored as an alias.) When no node sets this, the last node in the workflow is the responder by default. |
