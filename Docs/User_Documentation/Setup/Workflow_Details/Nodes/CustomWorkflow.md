...
| **`is_responder`**                  | Boolean          | No       | `false` | Determines if this node provides the final, user-facing response. If `true`, the child workflow's output is streamed to the user. If `false`, the output is captured as a variable for later nodes in the parent to use. (The original key name `returnToUser` is still honored as an alias.) When no node sets this, the last node in the workflow is the responder by default. |
...
| **`workflowUserFolderOverride`**    | String           | No       | `null`  | Specifies a user folder from which to load the `workflowName`. Use `_common` for workflows shared across all users. This overrides the user-level `workflowConfigsSubDirectoryOverride` setting if both are specified. |
